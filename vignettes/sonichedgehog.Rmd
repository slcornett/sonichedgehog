...
So many steps, right? Wouldn't it be nice to do less steps and arrive at the same endpoint? Here we are using the `Shh_fasta.f`, file (class = "character") version of `Shh_fasta` included in this package to run it through the {sonichedgehog} function `AlignAA()`. This function reads in converts the raw fasta file to an AAstring, does a multiple sequence alignment using the MUSCLE algorithm on the fasta data, then prints the complete set of aligned AA seqeuences with the species names and consensus sequence. Essentially, this combines by-hand steps 1-2. 
```{r pat}
...
ptree 
class(ptree)
# is there a way to use return without stopping the function? or is there a simple way to make a step of a function extractable without it being the last step of the function?
```
